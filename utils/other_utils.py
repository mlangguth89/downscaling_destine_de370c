...
def get_lcc_params(lon_sw: float, lat_sw: float, x_sw: float = 0.5, y_sw: float =0.5, lat0: float = 54., lon0: float =12.5, lat1: float = 30., 
                   lat2: float = 60., re=6370000., unit: str = "km"):
    
    """
    Get Lambert Conformal Conic projection false nrthing and false easting parameters (for EURAD-IM).
    :param lon_sw: longitude of the south-west corner of the domain (first grid point)
    :param lat_sw: latitude of the south-west corner of the domain (first grid point)
    :param x_sw: x-coordinate of the south-west corner of the domain (first grid point) 
    :param y_sw: y-coordinate of the south-west corner of the domain (first grid point) 
    :param lat0: latitude of natural origin or latitude of the projection origin
    :param lon0: central meridian
    :param lat1: first standard parallel
    :param lat2: second standard parallel
    :param re: earth radius
    :return: PROJ-string with false northing and false easting
    """
    # check unit
    if unit not in ["m", "km"]:
        raise ValueError("Unit must be either 'm' or 'km'.")
    
    # basic PROJ-string withouth false northing and false easting
    p_base = pyproj.Proj(f"+proj=lcc +lat_0={lat0} +lon_0={lon0} +lat_1={lat1} +lat_2={lat2} +x_0=0 +y_0=0 +R={re} +units={unit} +no_defs")
    off_set = p_base(lon_sw, lat_sw)
    off_set = -(np.array(off_set) - np.array([x_sw, y_sw]))
    if unit == "km":
        off_set = off_set * 1000
    
    # corrected PROJ-string with false northing and false easting
    proj_params = f"+proj=lcc +lat_0={lat0} +lon_0={lon0} +lat_1={lat1} +lat_2={lat2} +x_0={off_set[0]} +y_0={off_set[1]} +R={re} +units={unit} +no_defs"

    # check if the projection is correct
    p = pyproj.Proj(proj_params)
    assert np.allclose(p(lon_sw, lat_sw), [x_sw, y_sw]), "The projection is not correct."
    assert np.allclose(p(x_sw, y_sw, inverse=True), [lon_sw, lat_sw]), "The projection is not correct."

    return proj_params

def get_lcc_coords(x, y, proj_params):
    """
    Generate 2D lat-lon data from the (x,y)-coordinates of a regular (conic) Lambert Conformal grid.
    :param x: x-coordinates of the LC
    :param y: y-coordinate of the LC
    :param proj_params: dictionary of proj-parameters to describe LC-grid
                        The following keys are required:
                        - central_latitude
                        - central_longitude
                        - standard parallels [lat1, lat2]
                        - false easting
                        - false northing
                        - earth radius in metres
    :return: tuple of 2D (lon, lat)-fields
    """
    x, y = np.meshgrid(x , y, indexing="ij")

    nlon, nlat = np.shape(x)
    lon, lat = np.zeros((nlon, nlat)), np.zeros((nlon, nlat))

    p = pyproj.Proj(f"+proj=lcc +lat_0={proj_params['central_latitude']} +lon_0={proj_params['central_longitude']} +lat_1={proj_params['standard_parallels'][0]}" +
                    f" +lat_2={proj_params['standard_parallels'][1]} +x_0={proj_params['false_easting']} +y_0={proj_params['false_northing']} +R={proj_params['earth_radius']} +units=m +no_defs")

    for i in range(nlon):
        for j in range(nlat):
            lon[i, j], lat[i, j] = p(x[i,j], y[i,j], inverse=True) 
