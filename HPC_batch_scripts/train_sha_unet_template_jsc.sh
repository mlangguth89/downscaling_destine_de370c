...
#SBATCH --output=train_sha_unet_tier2-out.%j
#SBATCH --error=train_sha_unet_tier2-err.%j
...
# Parameters
# Set for Tier-2 dataset - Adapt accordingly to your dataset
indir=/p/scratch/deepacf/maelstrom/maelstrom_data/ap5/downscaling_benchmark_dataset/preprocessed_era5_crea6/t2m_mlevels/train   # for Tier-2 dataset
...
js_norm=${indir}/norm.json					# save computation time to get normalization parameters
...
srun --overlap python3 ${BASE_DIR}/main_scripts/main_train.py -in ${indir} -out ${outdir} -model ${model} -dataset ${dataset} \
	                                                           -conf_ds ${js_ds_conf} -conf_md ${js_model_conf} -exp_name ${exp_name} -id ${SLURM_JOBID} -js_norm ${js_norm}
