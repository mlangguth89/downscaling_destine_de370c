...
def decorate_plot(ax_plot, plot_xlabel=True, plot_ylabel=True, extent=[3.5, 16.5, 44.5, 54.]):
    ...
    ax_plot.set_xticks(np.arange(0., 360. + 0.1, 1.))  # ,crs=projection_crs)
    ax_plot.set_yticks(np.arange(-90., 90. + 0.1, 1.))  # ,crs=projection_crs)
...
def create_map_score(score, plt_fname, **kwargs):
    ...
    cbar = fig.colorbar(plt1, cax=cax, orientation="vertical", ticks=lvl[1:-1:1])
...
def create_line_plot(data: xr.DataArray, data_std: xr.DataArray, model_name: str, metric: dict,
    ...
    varname = kwargs.get("varname", "T2m")
...
def plot_downscaling_examples(in_data, tar_data, downscaled_data, lon, lat, fname, unit="ppbv", varname= "NOx", titles=["EURAD-IM 5km", "EURAD-IM 1km", "Downscaled EURAD-IM"], 
                              **kwargs):
    """
    Plot input, target and downscaled data side-by-side in a subplot with three columns. all data is expected to be provided on the same grid
    :param in_data: input data
    :param tar_data: griund truth data
    :param downscaled_data: data generated by downscaling model
    :param lon: 1D/2D-field of longitude data
    :param lat: 1D/2D-field of latitude data
    :param fname: filename of plot to be produced
    :param unit: unit of data
    :param varname: physical variable name
    :param titles: Titles for the three downscaling subplots
    :param kwargs: keyword-arguments to configure plot apparance
                   Valid keywords are:
                   - figsize: (x,y)-size of figure object in inches
                   - proj_data: cartopy projection object of the data
                   - proj_plot: cartopy projection object of the plot
                   - cmap_name: name of colormap used for plotting
                   - levels: levels for contour plot
                   -  other keyword arguments valid for ax.pcolormesh
    """    
    figsize = kwargs.pop("figsize", (18, 6))
    proj_data = kwargs.pop("proj_data", ccrs.PlateCarree())
    proj_plot = kwargs.pop("proj_plot", ccrs.PlateCarree())
    cmap_name = kwargs.pop("cmap", "RdBu_r")
    levels = kwargs.pop("levels", np.arange(0., 40.))
       
    # Create a figure and axis with the specified projection
    fig, axs = plt.subplots(1, 3, figsize=figsize, subplot_kw={'projection': proj_data})
    fig.patch.set_facecolor('white')

    # Plot the data using pcolormesh
    nbounds = len(levels)
    col_obj = plt.get_cmap(cmap_name)
    col_obj = col_obj(np.linspace(0., 1., nbounds)) 
    cmap_obj = mpl.colors.ListedColormap(col_obj, name=f"{varname}_map")
    norm = mpl.colors.BoundaryNorm(levels, cmap_obj.N)

    plt_data = [in_data, tar_data, downscaled_data]
    for i, ax in enumerate(axs):
        pcm = ax.pcolormesh(lon, lat, plt_data[i], transform=proj_plot, cmap=cmap_obj, norm=norm, **kwargs)
        # add borders 
        ax.gridlines()
        ax.add_feature(cfeature.BORDERS, linestyle='--', lw=3.)
        ax.add_feature(cfeature.STATES, linestyle='--', lw=1.)
        
        # set title
        ax.set_title(titles[i])
        # print maxima
        ax.text(0.5, -.05, f'Max: {np.max(plt_data[i]):.2f} {unit}', horizontalalignment='center', transform=ax.transAxes)
        ax.text(0.5, -.1, f'Min: {np.min(plt_data[i]):.2f} {unit}', horizontalalignment='center', transform=ax.transAxes)

    # Add colorbar...
    cbar = fig.colorbar(pcm, ax=axs, orientation='vertical', shrink=0.7)
    cbar.set_label(label=f'{varname} [{unit}]')

    # ... and show and save plot   
    print(f"Save plot to file '{fname}'")
    plt.savefig(fname)
    plt.show()
    plt.close()