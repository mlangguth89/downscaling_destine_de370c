...
class WGAN_Model(keras.Model):
    def __init__(self, generator, critic, hparams):
        ...
        # set critic_steps and gen_steps (modified in pre-training callback)
        self.step_var = K.variable(0, dtype="int32")
        self.pretrain = K.variable(self.hparams["pretraining_generator"], dtype="bool")
        #self.critic_steps = K.variable(self.hparams["d_steps"], dtype="int32")
        #self.gen_steps = K.variable(1, dtype="int32")
    ...
    def train_step(self, data_iter: tf.data.Dataset, embed=None) -> OrderedDict:
        ...
        critic_steps = self.hparams["d_steps"]
        gen_steps = self.hparams["d_steps"] + 1
        #cond = tf.logical_and(self.pretrain, self.step_var < self.hparams["pretrain_epochs"] * 1344)
        #critic_steps = tf.cond(cond, lambda: K.variable(0, dtype="int32"), lambda: K.variable(critic_steps, dtype="int32"))

        if self.pretrain and self.step_var < self.hparams["pretrain_epochs"]*1344:
            critic_steps = 0
        c_loss, gp, d_loss = np.nan, np.nan, np.nan

        # train the critic d_steps-times
        for i in tf.range(critic_steps):
        ...
        # train generator
        for i in range(critic_steps, gen_steps):
            with tf.GradientTape() as tape_generator:
                ...
                rloss = self.recon_loss(predictands[ist:ie, :, :, :], gen_data)
        ...
        # update step_var
        self.step_var.assign_add(delta=gen_steps)
...
class WGAN(AbstractModelClass):
    ...
    def set_hparams_default(self):
        ...
        """
        self.hparams_default = {"batch_size": 32, "nepochs": 60, "lr_decay": False, "decay_start": 5, "decay_end": 30, 
                                "l_embed": False, "d_steps": 5, "recon_weight": 1000., "gp_weight": 10., "optimizer": "adam", 
                                "pretraining_generator": False, "pretrain_epochs": 10,
